...
node<T>* BST<T> :: balance(node<T>* p) {
    fixHeight(p);
    int balanceFactorP = balanceFactor(p);
    if (abs(balanceFactorP)<2)
        return p; //NULL case handled
    else if (balanceFactorP > 0) { //left heavy
        if (balanceFactor(p->left) < 0) { //right heavy
            p->left = rotateleft(p->left);
            p = rotateright(p);
        } else 
            p = rotateright(p);
    } else if (balanceFactorP < 0) { //right heavy
        if (balanceFactor(p->right) > 0) { //left heavy
