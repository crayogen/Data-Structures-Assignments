...
void test(vector<string> allWords, vector<string> queries, promise<bool> done_future){
    ...
    duration<double> totalT = duration_cast<duration<double>>(tEnd - timeStart);
    cout << "inserted in : " << totalT.count() << " SECONDS."<<endl;

    // SEARCHING ALL THE ENTRIES
    for(int i=0;i<allWords.size();i++){
        if (map->lookup(allWords[i]) != NULL &&  map->lookup(allWords[i])->value != allWords[i]){
            cout << "TEST FAILED\n" << "LOOKUP FAILED" << endl;
        }
        if(timeOut){
            cout << "TEST FAILED\n" << "TIMED OUT"<<endl;
